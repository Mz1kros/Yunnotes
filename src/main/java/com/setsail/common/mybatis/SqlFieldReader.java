...
import com.setsail.common.mybatis.annotation.*;
...
public class SqlFieldReader {
    ...
    public static <T extends BaseEntity> String getFieldStr(T entity){
        ...
        for (Field declaredField : declaredFields) {
            allFields.append(declaredField.getName()).append(",");
            if(declaredField.getAnnotation(FieldAttribute.class) != null){
                FieldAttribute fieldAttribute = declaredField.getAnnotation(FieldAttribute.class);
                //如果查询明细字段，返回明细字段
    ...
    public static <T extends BaseEntity> String getConditionSuffix(T entity){
        ...
        try {
            for (Field declaredField : declaredFields) {
                if (declaredField.getAnnotation(IndexAttribute.class) != null){
                    if (SqlFieldReader.hasValue(entity, declaredField.getName())){
    ...
    public static <T> boolean hasValue(T entity,String fieldName){
        try {
            Class clazz = entity.getClass();
            Method method = clazz.getMethod("get" + StringUtils.captureName(fieldName));
